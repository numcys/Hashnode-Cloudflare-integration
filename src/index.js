...
const generateHtml = require('./templating');
...
if (request.url.pathname.startsWith('/hashnode')) {
  try {
    const posts = await fetchPostsFromHashnode();
    const html = await generateHtml(posts);
